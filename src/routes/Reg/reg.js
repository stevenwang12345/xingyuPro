...
class Reg extends React.Component {
    ...
    return (
    		<div>
				...
				<div className="reg-wrapper">
				    <Row>
				    	<Col span={6}></Col>
				    	<Col span={12} className="regs">
							<Form onSubmit={this.handleQuickSubmit.bind(this)} className="login-form">
							        <FormItem label="手机号" {...formItemLayout} >
							          {getFieldDecorator('phone', {
							            rules: [{ required: true, message: '请输入手机号' }],
							          })(
							            <Input className="reg-user-input" prefix={<Icon type="user" style={{ fontSize: 13 }} />} placeholder="手机号" />
							          )}
							        </FormItem>
							        <FormItem label="验证码" {...formItemLayout}>
							          <Row gutter={8}>
							          	...
							          	<Col span={8}>
							          		<img src={this.state.currentCodePath} onClick={this.changeCode.bind(this)} style={{height: '32px',marginTop: '0px'}}/>
							          	</Col>
							          	<Col span={8}>
								         ...
								         <Col span={12}>
								         	{
								         		...
								         		: <span className="ant-form-text freeGetcode djs djs-btn-style"><b>{this.state.currentSecond}</b>秒后重试</span> 
								         	}
							          	 </Col>
							          </Row>
							        </FormItem>
							        ...
							        <FormItem style={{ marginBottom: 8 }}>
							          {getFieldDecorator('agreement', {
							            ...
							            <p><Checkbox checked={this.state.check} onChange={this.onChangeAgree.bind(this)}></Checkbox>
										同意上海互联网金融评价中心 <a href="javascript:void(0)" onClick={this.showAgreement.bind(this)}>《用户协议》</a>
							            </p>
							          )}
							        </FormItem>
							        <FormItem>
							          ...
							          <p style={{textAlign: 'right', paddingRight: '120px'}}>已有账号,<a href="#/login">立即登录</a></p>
							        </FormItem>
						    </Form>
						    <Row>
						    	<Col span={5} offset={8}>
  ...
  componentDidMount() {
  	    window._hmt && window._hmt.push(['_trackEvent', '注册页', '点击','访问次数']);
		var timestamp = Date.parse(new Date()); 
		var url = HOST + 'api/client/member/v1.0/code/'+ timestamp;
		this.setState({
			currentCodePath: url
